...
export default function FeaturedToolCard({ name, description, url, logo, type, verified = false }: FeaturedToolCardProps) {
  return (
    <Link href={url} target="_blank" rel="noopener noreferrer">
      <div className="relative bg-gradient-to-r from-green-500 to-teal-600 text-white p-6 rounded-xl shadow-xl hover:scale-105 transition duration-300 cursor-pointer flex flex-col items-center">
        
        {/* Logo with Verification Icon */}
        <div className="relative w-16 h-16 mb-3">
          <Image src={logo} alt={name} width={64} height={64} className="rounded-full" />
          {verified && (
            <CheckCircle className="absolute -bottom-1 -right-1 text-white bg-green-500 rounded-full p-0.5" size={18} />
          )}
        </div>
